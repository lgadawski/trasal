...
public:
	...
	Individual(const Individual & copy) :
			map(copy.map), length(copy.length), seq(copy.seq), propability(
...
//		std::cout << " ~individual ";
	...
	void SetCumulativeDistribution(double acum_distr) {
		cum_dist = acum_distr;
	}
	double GetPropability() const {
		return propability;
	}
	void SetSeq(int aseq) {
		seq = aseq;
	}
	int GetSeq() const {
		return seq;
	...
	void CalculatePropability(int sum, int sum_division) {
		double d_sum = (double) sum, d_sum_divisiont = (double) sum_division,
				d_len = (double) GetLength();
