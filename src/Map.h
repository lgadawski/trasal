...
public:
	...
	void AddRandomCity(int id, int lowestPossibleDistance,
	...
	static std::shared_ptr<Map> ConstructMapOfSize(int mapSize,
			int lowestPossibleDistance, int highestPossibleDistance);

	std::string ToString();
	int getDistanceBetween(const City & c1, const City & c2);

	void print() {
		std::cout << std::endl << std::endl << "map size: " << getMapSize()
				<< std::endl;
		std::cout << "nodes:";
		for (std::set<City>::iterator it = citySet.begin(); it != citySet.end();
				it++) {
			std::cout << *it << " ";
		}
		std::cout << std::endl;
